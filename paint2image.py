...
from SinGAN.training import train_paint
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--paint_start_scale", help="paint injection scale", type=int, required=True
    )
    parser.add_argument(
        "--quantization_flag",
    ...
    #    logger.info("output already exist")
        ...
        if (opt.paint_start_scale < 1) | (opt.paint_start_scale > (len(Gs) - 1)):
            ...
            if opt.quantization_flag:
                ...
                plt.imsave(
                    "%s/real_quant.png" % dir2save,
                    functions.convert_image_np(real_quant),
                    vmin=0,
                    vmax=1,
                )
                plt.imsave(
                    "%s/in_paint.png" % dir2save,
                ...
                in_s = in_s[:, :, : reals[n].shape[2], : reals[n].shape[3]]
                plt.imsave(
                    "%s/in_paint_quant.png" % dir2save,
                    functions.convert_image_np(in_s),
                    vmin=0,
                    vmax=1,
                )
                if os.path.exists(dir2trained_model):
                    # logger.info('Trained model does not exist, training SinGAN for SR')
                    Gs, Zs, reals, NoiseAmp = functions.load_trained_pyramid(opt)
                    opt.mode = "paint2image"
                else:
                    train_paint(
                        opt, Gs, Zs, reals, NoiseAmp, centers, opt.paint_start_scale
                    )
                    opt.mode = "paint2image"
            out = SinGAN_generate(
            ...
            plt.imsave(
                "%s/start_scale=%d.png" % (dir2save, opt.paint_start_scale),
