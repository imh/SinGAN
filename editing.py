...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ref_dir", help="input reference dir", default="Input/Editing"
    )
    parser.add_argument("--ref_name", help="reference image name", required=True)
    parser.add_argument(
        "--editing_start_scale", help="editing injection scale", type=int, required=True
    )
    ...
    #    logger.info("output already exist")
        ...
        if (opt.editing_start_scale < 1) | (opt.editing_start_scale > (len(Gs) - 1)):
            ...
            plt.imsave(
                "%s/start_scale=%d.png" % (dir2save, opt.editing_start_scale),
            ...
            plt.imsave(
                "%s/start_scale=%d_masked.png" % (dir2save, opt.editing_start_scale),
                functions.convert_image_np(out.detach()),
                vmin=0,
                vmax=1,
            )
