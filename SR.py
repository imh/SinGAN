...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_name", help="training image name", default="33039_LR.png"
    )  # required=True)
    parser.add_argument(
    ...
    #    logger.info("output already exist")
        ...
        if os.path.exists(dir2trained_model):
            ...
            logger.info("*** Train SinGAN for SR ***")
            real = functions.read_image(opt)
            opt.min_size = 18
            real = functions.adjust_scales2image_SR(real, opt)
            train(opt, Gs, Zs, reals, NoiseAmp)
            opt.mode = mode
        logger.info("%f" % pow(in_scale, iter_num))
        Zs_sr = []
        reals_sr = []
        NoiseAmp_sr = []
        Gs_sr = []
        ...
        for j in range(1, iter_num + 1, 1):
            ...
            Zs_sr.append(z_opt)
        out = SinGAN_generate(
            Gs_sr, Zs_sr, reals_sr, NoiseAmp_sr, opt, in_s=reals_sr[0], num_samples=1
        )
        out = out[
            :,
            :,
        ...
        dir2save = functions.generate_dir2save(opt)
        plt.imsave(
            "%s/%s_HR.png" % (dir2save, opt.input_name[:-4]),
